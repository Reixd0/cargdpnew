...
st.text("Please follow the column labels from the format, label the column you would like to predict as 'Total GDP per Province'. Make sure that the years are in ascending order.")
...
st.header("CSV File Sample")

with st.expander("Show Sample CSV"):
    sample_df = pd.read_csv(samplecsv)#Load the CSV
    st.dataframe(sample_df)  # Display using st.dataframe
